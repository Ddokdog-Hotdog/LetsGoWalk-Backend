...
public class MemberController {
    ...
    public ResponseEntity<MemberDTO> completeRegistration(@Valid @RequestBody MemberUpdateDTO updateDTO, Long memberId){
	...
	MemberDTO updatedMember = memberService.updateMemberInfo(memberId, updateDTO);
    ...
    public ResponseEntity<MemberDTO> myInfoEdit(@Valid @RequestBody MemberUpdateDTO updateDTO, Long memberId) {
    	System.out.println("0");
    	if (memberId == null || updateDTO == null) {
            return ResponseEntity.badRequest().build(); // 입력 값이 null인 경우, Bad Request 응답을 반환
        }
